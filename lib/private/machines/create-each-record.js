module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    // }
    (async function doInsertMany(){
      try {
        var nativeResult = await mongoCollection.insertMany(s3q.newRecords);
        ...
        // Build records from the inserted ids by querying them back.
        var insertedIds = nativeResult && nativeResult.insertedIds ? Object.values(nativeResult.insertedIds) : [];
        var phRecords = [];
        if (insertedIds.length > 0) {
          phRecords = await mongoCollection.find({ _id: { $in: insertedIds } }).toArray();
        }
        try {
