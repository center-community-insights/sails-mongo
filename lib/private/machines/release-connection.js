...
module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    // If the connection doesn't appear to be a valid Mongo db/manager, catch it
    // ahead of time so we can provide a slightly nicer error message.
    // In the modern MongoDB driver (v4+), the db object doesn't have a .close() method;
    // instead, we store a reference to the MongoClient on db._mongoClient.
    if (!_.isObject(inputs.connection)) {
      return exits.badConnection();
    }
    // Check for either legacy db.close() or modern db._mongoClient.close().
    var hasLegacyClose = _.isFunction(inputs.connection.close);
    var hasModernClient = _.isObject(inputs.connection._mongoClient) && _.isFunction(inputs.connection._mongoClient.close);
    if (!hasLegacyClose && !hasModernClient) {
      return exits.badConnection();
    }
