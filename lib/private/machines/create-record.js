  ...
  fn: function (inputs, exits) {
    ...
    (async ()=>{
      ...
      if (!isFetchEnabled) {
        ...
        try { attachSaveShim(s3q.newRecord, mongoCollection, WLModel, s3q.meta); } catch (unusedErr) { /* ignore */ }
      ...
      // If the model's PK is NOT `_id` (e.g. connect-waterline sessions uses `sid` as PK),
      // then we must refetch using the PK value we inserted, NOT Mongo's generated `_id`.
      //
      // Otherwise we'd end up querying something like `{ sid: ObjectId(...) }` and fail to refetch.
      if (pkColumnName !== '_id' && !_.isUndefined(s3q.newRecord[pkColumnName])) {
        criteria[pkColumnName] = s3q.newRecord[pkColumnName];
      } else {
      ...
      // Backwards-compatibility: attach legacy `.save()` for nxus / older Waterline patterns.
      try { attachSaveShim(phRecord, mongoCollection, WLModel, s3q.meta); } catch (unusedErr) { /* ignore */ }
