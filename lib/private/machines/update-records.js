module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    //   ║║║╣  ║ ║╣ ╠╦╝║║║║║║║║╣   │││├─┤├┤  │ ├─┤├┤ ├┬┘   │ │ │  ╠╣ ║╣  ║ ║  ╠═╣  │ │├┬┘  ││││ │ │
    //  ═╩╝╚═╝ ╩ ╚═╝╩╚═╩ ╩╩╝╚╝╚═╝  └┴┘┴ ┴└─┘ ┴ ┴ ┴└─┘┴└─   ┴ └─┘  ╚  ╚═╝ ╩ ╚═╝╩ ╩  └─┘┴└─  ┘└┘└─┘ ┴
    // Waterline 0.12 (nxus, sails <1.0) does NOT pass `meta.fetch` and expects records to be returned.
    // Default to fetch:true for backwards compatibility; only disable if explicitly set to false.
    var isFetchEnabled;
    if (s3q.meta && s3q.meta.fetch === false) { isFetchEnabled = false; }
    else { isFetchEnabled = true; }
    ...
    (async ()=>{
      var pkValsOfMatchingRecords;
      if (isFetchEnabled) {
        var projection = {};
        projection[pkColumnName] = 1;
        var nativeResult = await mongoCollection.find(mongoWhere, { projection: projection }).toArray();
        pkValsOfMatchingRecords = _.pluck(nativeResult, pkColumnName);
      }
      ...
      // MongoDB doesn't allow updating the _id field, so remove it from valuesToSet.
      // Also remove the pk column name (e.g. `id`) which might be aliased to `_id`.
      var valuesToSetForMongo = _.omit(s3q.valuesToSet, ['_id', pkColumnName]);

      try {
        await mongoCollection.updateMany(secondaryMongoWhere, { '$set': valuesToSetForMongo });
      } catch (err) {
      ...
      var phRecords = await mongoCollection.find(secondaryMongoWhere).toArray();
