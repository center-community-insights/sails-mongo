module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    (async ()=>{
      ...
      db._sailsMongoCloseClient = client.close.bind(client);
      // Also protect `db.client.close()` (Db exposes `.client`), since some wrappers call it directly.
      // Keep the real close around so destroy-manager can still shut down cleanly.
      client._sailsMongoCloseClient = db._sailsMongoCloseClient;
      client.close = function () {
        // Only allow actually closing the client when the adapter is tearing down.
        if (db._sailsMongoAllowClose || client._sailsMongoAllowClose) {
          return client._sailsMongoCloseClient();
        }

        var cb = arguments.length && typeof arguments[arguments.length - 1] === 'function' ? arguments[arguments.length - 1] : undefined;
        if (!db._sailsMongoWarnedCloseClient) {
          db._sailsMongoWarnedCloseClient = true;
          // eslint-disable-next-line no-console
          console.warn(
            'Warning: `.close()` was called on a MongoClient managed by sails-mongo (e.g. via `manager.client.close()`).  This adapter uses a shared MongoClient\n'+
            'for the datastore, so closing it from userland is not supported.  Use Waterline/adapter teardown to manage shutdown.  (Ignoring this close call.)'
          );
        }
      ...
      db.close = function () {
        // Only allow actually closing the client when the adapter is tearing down.
        if (db._sailsMongoAllowClose) {
          return db._sailsMongoCloseClient();
        }
        if (!db._sailsMongoWarnedClose) {
          db._sailsMongoWarnedClose = true;
          // eslint-disable-next-line no-console
          console.warn(
            'Warning: `.close()` was called on a sails-mongo manager/connection.  This adapter uses a shared MongoClient\n'+
            'for the datastore, so calling `.close()` from userland is not supported.  Use `sails.getDatastore().leaseConnection()`\n'+
            'without manually closing, or allow Waterline teardown to manage shutdown.  (Ignoring this close call.)'
          );
        }
        return Promise.resolve();
      };
