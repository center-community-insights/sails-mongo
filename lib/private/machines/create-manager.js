module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    // Also strip old Waterline config properties that are not MongoDB driver options.
    _.each([
      'reconnectTries',
      'reconnectInterval',
      'autoReconnect',
      ...
      'reconnectinterval',
      'autoreconnect',
      // Legacy Waterline 0.10.x/0.12.x config properties (not MongoDB options)
      'version',
      'schema',
      'defaults',
      'name',
    ...
    // In MongoDB driver v4+, we must use `new MongoClient()` then `client.connect()`,
    // and then get the `db` via `client.db()`.
    (async ()=>{
      ...
      // Store a reference to the underlying MongoClient on the db, so that destroyManager
      // can close it properly.  In the legacy driver, `db.close()` was the way to close;
      // in the modern driver, it's `client.close()`.
