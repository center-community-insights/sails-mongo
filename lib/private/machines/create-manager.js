module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    (async function connectAndBuildManager(){
      try {
        var MongoClient = NodeMongoDBNativeLib.MongoClient || NodeMongoDBNativeLib;
        var client = new MongoClient(mongoUrl, _clientConfig);
        await client.connect();

        // Derive the database name from the URL path (if present).
        var parsed;
        var dbName;
        try {
          parsed = new (require('url').URL)(mongoUrl);
          dbName = (parsed.pathname || '').replace(/^\//, '') || undefined;
        } catch (unusedErr) {
          dbName = undefined;
        }

        // Use the db name from the URL if available; otherwise, let the driver default.
        var db = dbName ? client.db(dbName) : client.db();

        // Shim a close() on the Db instance so existing code paths can close the client.
        // (Db instances do not expose close(); MongoClient does.)
        ...
        return exits.failed({ error: err, meta: inputs.meta });
      }
    })();
  }
