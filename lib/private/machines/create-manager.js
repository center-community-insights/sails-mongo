module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    (async ()=>{
      ...
      console.log('[sails-mongo] MongoClient connected successfully. Close protection enabled.');
      ...
      // We keep that contract, but we also need a way for Waterline to tear down the underlying MongoClient.
      //
      // NOTE:
      // Some userland code incorrectly calls `.close()` on the manager/leasedConnection, thinking it's a per-query
      // connection handle.  In this adapter, closing the underlying client will break subsequent ORM queries and can
      // surface as `MongoExpiredSessionError: Cannot use a session that has ended`.
      //
      // So: provide a safe `.close()` that is a no-op for userland, and keep the real close fn on a private property
      // for use by this adapter's `destroyManager()` implementation.

      // Store the REAL close function in a closure variable (not on the object, so it can't be accessed).
      var realClientClose = client.close.bind(client);

      // Create a shared state object for the adapter to control teardown.
      var adapterState = {
        allowClose: false,
        warnedClose: false,
        warnedCloseClient: false
      };

      // Helper to create a safe close function that blocks userland calls.
      var createSafeClose = function (label) {
        return function safeClose() {
          // Only allow actually closing the client when the adapter is tearing down.
          if (adapterState.allowClose) {
            return realClientClose();
          }

          var cb = arguments.length && typeof arguments[arguments.length - 1] === 'function' ? arguments[arguments.length - 1] : undefined;
          if (!adapterState.warnedClose) {
            adapterState.warnedClose = true;
            // eslint-disable-next-line no-console
            console.warn(
              'Warning: `.close()` was called on a MongoClient managed by sails-mongo (via '+label+').\n'+
              'This adapter uses a shared MongoClient for the datastore, so closing it from userland\n'+
              'is not supported.  Use Waterline/adapter teardown to manage shutdown.  (Ignoring this close call.)\n'+
              'Stack trace:\n'+(new Error()).stack
            );
          }
          if (cb) { try { cb(); } catch (unusedErr) { /* ignore */ } }
          return Promise.resolve();
        };
      };

      // Replace close on the client itself.
      client.close = createSafeClose('client.close');

      // Also protect internal topology close if present (MongoDB driver internals).
      if (client.topology && typeof client.topology.close === 'function') {
        var realTopologyClose = client.topology.close.bind(client.topology);
        client.topology.close = function () {
          if (adapterState.allowClose) { return realTopologyClose.apply(this, arguments); }
          if (!adapterState.warnedClose) {
            adapterState.warnedClose = true;
            // eslint-disable-next-line no-console
            console.warn(
              'Warning: `.close()` was called on MongoClient topology (internal).\n'+
              'This adapter uses a shared MongoClient.  (Ignoring this close call.)\n'+
              'Stack trace:\n'+(new Error()).stack
            );
          }
          return Promise.resolve();
      ...
      // Add event listeners to detect when the client/topology closes unexpectedly.
      // This helps debug cases where close happens through an unprotected path.
      try {
        client.on('close', function () {
          if (!adapterState.allowClose) {
            // eslint-disable-next-line no-console
            console.error(
              '[sails-mongo] MongoClient emitted "close" event but adapter teardown was NOT in progress!\n'+
              'This indicates something closed the connection unexpectedly.\n'+
              'Stack trace:\n'+(new Error()).stack
            );
          }
        });
        if (client.topology) {
          client.topology.on('close', function () {
            if (!adapterState.allowClose) {
              // eslint-disable-next-line no-console
              console.error(
                '[sails-mongo] MongoClient topology emitted "close" event but adapter teardown was NOT in progress!\n'+
                'This indicates something closed the connection unexpectedly.\n'+
                'Stack trace:\n'+(new Error()).stack
              );
            }
      ...
      } catch (unusedErr) { /* ignore if events not supported */ }

      // Replace close on the db handle.
      db.close = createSafeClose('db.close');

      // Store adapter state and real close on the db so destroyManager can access them.
      db._mongoClient = client;
      db._sailsMongoAdapterState = adapterState;
      db._sailsMongoCloseClient = function () {
        adapterState.allowClose = true;
        return realClientClose();
      };
