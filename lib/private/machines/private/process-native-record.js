...
module.exports = function processNativeRecord(nativeRecord, WLModel, meta) {
  ...
  // Ensure the physical PK column is present.
  // Mongo always uses `_id` as the real identifier, but depending on how a model was normalized
  // upstream, Waterline might think the PK column name is something else (e.g. `id`).
  // If the expected PK column is missing but `_id` is present, alias it in.
  var primaryKeyColumnNameForModel = WLModel.attributes[WLModel.primaryKey].columnName;
  if (_.isUndefined(nativeRecord[primaryKeyColumnNameForModel]) && !_.isUndefined(nativeRecord._id)) {
    nativeRecord[primaryKeyColumnNameForModel] = nativeRecord._id;
  }

  // Compatibility shim:
  // Mongo stores the identifier in `_id`.  Some Waterline setups (notably older nxus configs) may not
  // have `columnName: '_id'` set up for the PK, so Waterline's transformer expects a physical `id` field.
  // If we find `_id` but not `id`, alias it so Waterline can hydrate a model instance with a PK value.
  if (_.isUndefined(nativeRecord.id) && !_.isUndefined(nativeRecord._id)) {
    nativeRecord.id = nativeRecord._id;
  }

  // Ensure `_id` / `id` are stringified consistently for ObjectId-backed models.
  if (useObjectIds) {
    if (_.isObject(nativeRecord._id) && nativeRecord._id instanceof ObjectId) {
      nativeRecord._id = nativeRecord._id.toString();
    }
    if (_.isObject(nativeRecord.id) && nativeRecord.id instanceof ObjectId) {
      nativeRecord.id = nativeRecord.id.toString();
    }
  }
