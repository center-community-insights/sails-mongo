...
module.exports = function processNativeRecord(nativeRecord, WLModel, meta) {
  ...
  // Compatibility shim:
  // In some Waterline setups (notably legacy nxus-storage configs), the primary key column name may be `id`
  // even though Mongo stores the actual identifier in `_id`.  If we find `_id` but not `id`, alias it so
  // Waterline's transformer can still hydrate a model instance with a primary key value.
  if (_.isUndefined(nativeRecord.id) && !_.isUndefined(nativeRecord._id)) {
    var pkAttrDefMaybe = WLModel.attributes && WLModel.attributes[WLModel.primaryKey];
    var pkColumnNameMaybe = pkAttrDefMaybe && pkAttrDefMaybe.columnName;
    if (pkColumnNameMaybe === 'id' || (!pkColumnNameMaybe && WLModel.primaryKey === 'id')) {
