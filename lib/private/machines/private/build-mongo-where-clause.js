...
module.exports = function buildMongoWhereClause(whereClause, WLModel, meta) {
  ...
  whereClause = (function cloneDeepPreservingInstances(x) {
    ...
    // Preserve/normalize ObjectId-like instances (including "foreign" bson ObjectIds from other deps).
    if (x && x._bsontype === 'ObjectId') {
      try { return normalizeMongoObjectId(x); } catch (unusedErr) { /* ignore */ }
    }
  ...
  var mongoQueryFilter = (function recurse(branch) {
    ...
    if (
      ...
      // Treat foreign ObjectId instances as scalar eq constraints too.
      (constraint && constraint._bsontype === 'ObjectId') ||
      (constraint instanceof Date) ||
      (constraint instanceof RegExp)
    ) {

      // Normalize foreign ObjectId instances into this adapter's bson implementation.
      if (constraint && constraint._bsontype === 'ObjectId' && !_.isString(constraint)) {
        try {
