...
var normalizeBson = require('./normalize-bson');
...
module.exports = function buildMongoWhereClause(whereClause, WLModel, meta) {
  ...
  whereClause = (function cloneDeepPreservingInstances(x) {
    ...
    // Normalize any foreign bson instances into this adapter's bson implementation.
    // This prevents BSONVersionError when mongodb@6 driver tries to serialize foreign BSON types.
    if (x && x._bsontype) {
      // normalizeBson will throw if it can't convert a foreign ObjectId.
      // Let that error propagate so we get a clear message instead of a cryptic BSONVersionError.
      return normalizeBson(x);
    }
  ...
  var mongoQueryFilter = (function recurse(branch) {
    ...
    if (
      ...
      // Treat foreign bson instances as scalar eq constraints too.
      (constraint && constraint._bsontype) ||
      (constraint instanceof Date) ||
      (constraint instanceof RegExp)
    ) {

      // Normalize foreign bson instances into this adapter's bson implementation.
      if (constraint && constraint._bsontype && !_.isString(constraint)) {
        try { branch[constraintColumnName] = normalizeBson(constraint); } catch (e) { /* ignore */ }
        return branch;
      }
    ...
    // Backwards compatibility:
    // Normalize any foreign bson instance used as a modifier value.
    if (modifier && _.isObject(modifier) && modifier._bsontype) {
      try { modifier = normalizeBson(modifier); } catch (unusedErr) { /* ignore */ }
    }
    ...
    switch (modifierKind) {
      ...
      case '!=':
        ...
        // Also normalize foreign ObjectId-like values (from other bson versions).
        if (modifier && _.isObject(modifier) && modifier._bsontype) {
          try { modifier = normalizeBson(modifier); } catch (unusedErr) { /* ignore */ }
        }
      ...
      case 'nin':
        ...
        modifier = _.reduce(modifier, function (memo, item) {
          // Normalize any foreign bson instances inside arrays (prevents BSONVersionError).
          if (item && _.isObject(item) && item._bsontype) {
            try { item = normalizeBson(item); } catch (unusedErr) { /* ignore */ }
          }
