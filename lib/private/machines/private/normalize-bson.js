...
function normalizeBson(x) {
  ...
  if (x._bsontype === 'ObjectId') {
    ...
    try {
      ...
      // If we still can't convert, log a warning and throw a clear error
      // instead of returning the foreign BSON which causes cryptic BSONVersionError.
      if (process.env.SAILS_MONGO_DEBUG_FOREIGN_BSON) {
        // eslint-disable-next-line no-console
        console.error('[sails-mongo] Failed to normalize foreign ObjectId:', x, conversionErr);
      }
      throw new Error(
        'Cannot normalize foreign ObjectId from a different bson version. ' +
        'This usually happens when userland code creates ObjectId instances using a different ' +
        'version of the mongodb/bson package than sails-mongo uses. ' +
        'Original error: ' + (conversionErr && conversionErr.message)
      );
    }
  }
...
normalizeBson.deep = function deepNormalizeBson(x, path) {
  ...
  if (x && x._bsontype) {
    ...
    var bt = x._bsontype;
    var normalized = x;
    try { normalized = normalizeBson(x); } catch (unusedErr) { /* ignore */ }

    if (process.env.SAILS_MONGO_DEBUG_FOREIGN_BSON && normalized !== x) {
      // eslint-disable-next-line no-console
      console.warn('[sails-mongo] Normalized foreign BSON at '+(path || '(root)')+'  _bsontype='+bt+'  from='+beforeCtor+'  to='+(normalized && normalized.constructor && normalized.constructor.name));
    }
