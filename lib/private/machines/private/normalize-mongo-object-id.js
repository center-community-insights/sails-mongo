  ...
  // Detect ObjectId-like objects and convert them into *this adapter's* ObjectId implementation.
    ...
    if (_.isString(hex) && hex.length === 24 && /^[0-9a-fA-F]{24}$/.test(hex)) {
      try {
        ...
        // ObjectId construction failed - throw with context
        throw flaverr('E_CANNOT_INTERPRET_AS_OBJECTID', new Error(
          'Extracted hex "' + hex + '" from foreign ObjectId, but ObjectId constructor failed: ' + constructErr.message
        ));
      }
    }
    
    // Couldn't extract valid hex from foreign ObjectId
    throw flaverr('E_CANNOT_INTERPRET_AS_OBJECTID', new Error(
      'Cannot normalize foreign ObjectId - could not extract valid 24-character hex string.\n' +
      'Foreign ObjectId info: _bsontype=' + supposedId._bsontype + ', constructor=' + (supposedId.constructor && supposedId.constructor.name) + '\n' +
      'Attempted hex extraction result: ' + hex + '\n' +
      'This usually happens when userland code uses a different mongodb/bson version than sails-mongo.\n' +
      'Fix: Use require("sails-mongo").mongodb.ObjectId instead of importing ObjectId from mongodb directly.'
    ));
  }
