...
module.exports = function reifyValuesToSet(valuesToSet, WLModel, meta) {
  ...
  // This is a best-effort deep walk that preserves Buffers and other scalar instances.
  (function normalizeDeep(x) {
    if (_.isNull(x) || _.isUndefined(x)) { return x; }
    if (!_.isObject(x)) { return x; }
    if (x && x._bsontype) {
      try { return normalizeBson(x); } catch (unusedErr) { return x; }
    }
    if (Buffer && Buffer.isBuffer && Buffer.isBuffer(x)) { return x; }
    if (_.isArray(x)) {
      return _.map(x, normalizeDeep);
    }
    _.each(_.keys(x), function (k) {
      x[k] = normalizeDeep(x[k]);
    });
    return x;
  })(valuesToSet);
