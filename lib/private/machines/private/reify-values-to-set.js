...
module.exports = function reifyValuesToSet(valuesToSet, WLModel, meta) {
  ...
  // Normalize any foreign bson instances in values-to-set into this adapter's bson implementation,
  // otherwise mongodb@6 driver may throw BSONVersionError when attempting to serialize the update doc.
  // Use the comprehensive normalizeBson.deep() which handles all BSON types and throws
  // clear errors if normalization fails (instead of causing cryptic BSONVersionError later).
  normalizeBson.deep(valuesToSet);
