...
module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    // Resolve the real client close fn (prefer the private one attached by create-manager.js).
    if (!_.isObject(inputs.manager)) {
      return exits.error(new Error('The provided `manager` is not a valid manager created by this driver.'));
    }
    var closeFn =
      (_.isFunction(inputs.manager._sailsMongoCloseClient) && inputs.manager._sailsMongoCloseClient) ||
      (_.isObject(inputs.manager._mongoClient) && _.isFunction(inputs.manager._mongoClient.close) && inputs.manager._mongoClient.close.bind(inputs.manager._mongoClient)) ||
      (_.isFunction(inputs.manager.close) && inputs.manager.close);
    if (!closeFn) {
      return exits.error(new Error('The provided `manager` is not a valid manager created by this driver.  (It should expose a way to close the underlying MongoClient.)'));
    }

    // Call close on the manager (supports both sync and promise-returning variants).
    Promise.resolve()
    .then(function (){
      // Ensure the safe `.close()` (if present) actually closes during teardown.
      // The new pattern uses _sailsMongoAdapterState.allowClose; also support legacy flag for compat.
      try {
        if (inputs.manager._sailsMongoAdapterState) {
          inputs.manager._sailsMongoAdapterState.allowClose = true;
        }
      } catch (unusedErr) { /* ignore */ }
      try { inputs.manager._sailsMongoAllowClose = true; } catch (unusedErr) { /* ignore */ }
      try {
        if (inputs.manager._mongoClient) { inputs.manager._mongoClient._sailsMongoAllowClose = true; }
      } catch (unusedErr) { /* ignore */ }
      return closeFn();
    })
    .then(function (){
