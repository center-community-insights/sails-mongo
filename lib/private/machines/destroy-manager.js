...
module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    // In the modern MongoDB driver (v4+), the manager is the `db` object, and we store
    // a reference to the underlying `MongoClient` on `db._mongoClient`.
    // We need to close the MongoClient, not the db.
    var closeFn;

    // Prefer _mongoClient.close() if available (modern driver pattern).
    if (_.isObject(inputs.manager._mongoClient) && _.isFunction(inputs.manager._mongoClient.close)) {
      closeFn = inputs.manager._mongoClient.close.bind(inputs.manager._mongoClient);
    }
    // Fallback to manager.close() for legacy compatibility.
    else if (_.isFunction(inputs.manager.close)) {
      closeFn = inputs.manager.close.bind(inputs.manager);
    }
