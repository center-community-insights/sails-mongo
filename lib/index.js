...
module.exports = {
  ...
  teardown: function (datastoreName, done) {
    ...
    var _cleanupRegistryForEntry = function (entry) {
      ...
      try {
        _.each(_.keys(registeredDryModels), function(modelIdentity) {
          ...
          if (!m) { return; }
          if (m.datastore === datastoreName || dsNames.indexOf(m.datastore) !== -1) {
    ...
    if (!dsEntry.manager) {
      // If initialization failed, there's no manager to destroy; just clean up registry state.
      if (dsEntry.managerInitError) {
        _cleanupRegistryForEntry(dsEntry);
        return done();
      }
      // If initialization is still in-flight, wait for it to settle and then try teardown again.
      if (dsEntry.managerPromise && typeof dsEntry.managerPromise.then === 'function') {
        return dsEntry.managerPromise.then(function () {
          return module.exports.teardown(datastoreName, done);
        }).catch(function (err) {
          // Init failed; clean up and consider teardown complete.
          dsEntry.managerInitError = err;
          _cleanupRegistryForEntry(dsEntry);
          return done();
        });
    ...
    WET_MACHINES.destroyManager({ manager: dsEntry.manager }).switch({
      ...
      success: function (report) {
        ...
        // Remove this datastore from the in-memory registry (including any aliases).
        _cleanupRegistryForEntry(dsEntry);
