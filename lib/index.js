...
module.exports = {
  ...
  registerDatastore: function (dsConfig, physicalModelsReport, done) {
    ...
    // Register a placeholder datastore entry immediately, so that adapter methods can find it
    // even if userland code attempts queries before the asynchronous connection manager has
    ...
    var dsEntry = {
      ...
      managerPromise: null,
      // Track in-flight ops so teardown can wait briefly and avoid closing mid-query (MongoExpiredSessionError).
      _sailsMongoActiveOps: 0,
      _sailsMongoActiveOpsWaiters: []
    };
    registeredDsEntries[datastoreName] = dsEntry;
    // Resolve any pending waiters now that the datastore entry exists.
    try { waiter.resolve(dsEntry); } catch (unusedErr) { /* ignore */ }

    // Kick off manager creation and stash the promise (for any early queries).
    dsEntry.managerPromise = new Promise(function (resolve, reject) {
      WET_MACHINES.createManager({
        connectionString: dsConfig.url,
        meta: _.omit(dsConfig, ['adapter', 'url', 'identity', 'schema'])
      }).switch({
        error: function(err) {
          return reject(new Error('Consistency violation: Unexpected error creating db connection manager:\n```\n'+err.stack+'\n```'));
        },
    ...
    dsEntry.managerPromise
    .then(function (report) {
      try {

        var manager = report.manager;

        // Populate the placeholder entry with the real manager now that it is ready.
        dsEntry.manager = manager;

        // Register common aliases, since some wrappers pass different identifiers at query time.
        // (For example, Waterline "connections" legacy config sometimes uses connection names like "default".)
        _.each([dsConfig.connectionName, dsConfig.connection, dsConfig.name, dsConfig.datastoreName, dsConfig.datastore], function (alias) {
          if (!alias || alias === datastoreName) { return; }
          if (!registeredDsEntries[alias]) {
            registeredDsEntries[alias] = dsEntry;
          }
        });
        ...
        // Mark this datastore as failed, but keep the entry around so any in-flight / subsequent
        // queries fail with the *real* error (instead of "no matching datastore entry").
        dsEntry.managerInitError = e;

        // Best-effort cleanup if the manager was created before the failure happened.
        if (dsEntry.manager) {
          return WET_MACHINES.destroyManager({ manager: dsEntry.manager }).switch({
            error: function () { return done(e); },
            failed: function () { return done(e); },
            success: function () { return done(e); }
          });
        }
    ...
    .catch(function (err) {
      // Mark this datastore as failed, but keep the placeholder entry around so any in-flight / subsequent
      // queries fail with the *real* connection/config error (instead of "no matching datastore entry").
      dsEntry.managerInitError = err;
      return done(err);
    });
...
// =====================================================================================
// LEGACY WATERLINE 0.12.x COMPATIBILITY
// =====================================================================================
// Older Waterline versions (used by nxus-storage, etc.) call `registerConnection` instead of
// `registerDatastore`.  Modern Waterline (0.13+) explicitly checks for this method and throws
// if it exists (as a way to force adapter authors to migrate).
//
// To support BOTH versions:
// - We define `registerConnection` as a non-enumerable, non-configurable property.
// - Modern Waterline's check uses `_.isFunction(adapter.registerConnection)` which WILL see it,
//   so we can't avoid that check entirely.  Instead, the solution is to NOT add this method
//   when used with modern Waterline.
//
// Detect which Waterline version is in use by checking if the calling code actually invokes
// `registerConnection`.  If it does, we're on old Waterline; if not, we're on new.
//
// ALTERNATIVE APPROACH: Only add this when the ENV variable SAILS_MONGO_LEGACY_WATERLINE=1 is set,
// or when the consuming framework explicitly opts in.
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
if (process.env.SAILS_MONGO_LEGACY_WATERLINE === '1' || process.env.SAILS_MONGO_LEGACY_WATERLINE === 'true') {
  module.exports.registerConnection = function registerConnection(connectionConfig, physicalModelsReport, done) {
    ...
    return module.exports.registerDatastore(dsConfig, physicalModelsReport, done);
  };
}
