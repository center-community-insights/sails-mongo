  ...
  registerDatastore: function (dsConfig, physicalModelsReport, done) {
    ...
    .then(function (report) {
      try {
        ...
        // Clean up placeholder entry on error.
        try { delete registeredDsEntries[datastoreName]; } catch (unusedErr) { /* ignore */ }
        return done(e);
      }
    ...
    .catch(function (err) {
      // Clean up placeholder entry on failure.
      try { delete registeredDsEntries[datastoreName]; } catch (unusedErr) { /* ignore */ }
      // Reject any waiters so early queries fail with the real error (instead of hanging).
      try { waiter.reject(err); } catch (unusedErr) { /* ignore */ }
      return done(err);
    });
