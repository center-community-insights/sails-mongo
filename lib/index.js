...
module.exports = {
  ...
  registerDatastore: function (dsConfig, physicalModelsReport, done) {
    ...
    if (!datastoreName) {
      try {
        var parsedUrl = require('url').parse(dsConfig.url || '');
        var host = parsedUrl.hostname || 'default';
        var dbPath = (parsedUrl.pathname || '').replace(/^\/+/, '') || 'default';
        datastoreName = host + ':' + dbPath;
      } catch (e) {
  ...
  define: function (datastoreName, tableName, phmDef, done) {
    ...
    async.each(uniqueIndexesToCreate, function (key, next) {
      ...
      (async function doCreateIndex(){
        try {
          await mongoCollection.createIndex(mongoSingleFieldIdxKeys, { unique: true });
          return next();
        } catch (err) {
          if (!_.isError(err)) {
  ...
  drop: function (datastoreName, tableName, unused, done) {
    ...
    (async function doDrop(){
      try {
        var existed = await db.collection(tableName).drop();
        // existed === true if dropped, false if namespace not found in newer driver
        return done();
      } catch (err) {
        try {
          if (err && (err.codeName === 'NamespaceNotFound' || err.message === 'ns not found')) {
            throw flaverr('E_PHM_NOT_FOUND', new Error('No such physical model is currently defined.'));
          }
          if (_.isError(err)) { throw err; }
          throw new Error('Consistency violation: Expecting Error instance, but instead got: '+util.inspect(err));
        } catch (e) {
          switch (e.code) {
            case 'E_PHM_NOT_FOUND': return done();
            default:
              e.raw = err;
              return done(e);
          }
