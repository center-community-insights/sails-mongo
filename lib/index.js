...
module.exports = {
  ...
  native: function (unusedDatastoreName, unusedTableName, unusedCb) {
    ...
    // Also normalize foreign BSON instances to prevent BSONVersionError.
    // Wrap collection.find() and collection.findOne() to convert the old API.
    var normalizeBson = require('./private/machines/private/normalize-bson');
    
    // Helper to normalize and log errors
    var safeNormalize = function(obj, context) {
      try {
        return normalizeBson.deep(obj);
      } catch (e) {
        // eslint-disable-next-line no-console
        console.error('[sails-mongo] Failed to normalize BSON in ' + context + ':', e.message);
        console.error('[sails-mongo] Query/doc that failed:', JSON.stringify(obj, function(k, v) {
          if (v && v._bsontype) return '[' + v._bsontype + ': ' + (v.toString ? v.toString() : 'unknown') + ']';
          return v;
        }, 2));
        return obj; // Return original and let MongoDB give its own error
      }
    };
    
    var originalFind = collection.find.bind(collection);
    collection.find = function(query, options) {
      // Normalize any foreign BSON instances in the query to prevent BSONVersionError.
      query = safeNormalize(query, 'find query on ' + tblName);
    ...
    collection.findOne = function(query, options) {
      // Normalize any foreign BSON instances in the query to prevent BSONVersionError.
      query = safeNormalize(query, 'findOne query on ' + tblName);
      if (options && options.fields && !options.projection) {
        options = _.extend({}, options, { projection: options.fields });
        delete options.fields;
      }
    ...
    // Also wrap updateOne, updateMany, deleteOne, deleteMany for consistency.
    var originalUpdateOne = collection.updateOne.bind(collection);
    collection.updateOne = function(filter, update, options) {
      filter = safeNormalize(filter, 'updateOne filter on ' + tblName);
      update = safeNormalize(update, 'updateOne update on ' + tblName);
      return originalUpdateOne(filter, update, options);
    };

    var originalUpdateMany = collection.updateMany.bind(collection);
    collection.updateMany = function(filter, update, options) {
      filter = safeNormalize(filter, 'updateMany filter on ' + tblName);
      update = safeNormalize(update, 'updateMany update on ' + tblName);
      return originalUpdateMany(filter, update, options);
    };

    var originalDeleteOne = collection.deleteOne.bind(collection);
    collection.deleteOne = function(filter, options) {
      filter = safeNormalize(filter, 'deleteOne filter on ' + tblName);
      return originalDeleteOne(filter, options);
    };

    var originalDeleteMany = collection.deleteMany.bind(collection);
    collection.deleteMany = function(filter, options) {
      filter = safeNormalize(filter, 'deleteMany filter on ' + tblName);
      return originalDeleteMany(filter, options);
    };

    var originalInsertOne = collection.insertOne.bind(collection);
    collection.insertOne = function(doc, options) {
      doc = safeNormalize(doc, 'insertOne doc on ' + tblName);
      return originalInsertOne(doc, options);
    };

    var originalInsertMany = collection.insertMany.bind(collection);
    collection.insertMany = function(docs, options) {
      docs = safeNormalize(docs, 'insertMany docs on ' + tblName);
      return originalInsertMany(docs, options);
    };
